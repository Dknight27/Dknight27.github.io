...
区块链系统的可溯源不可篡改特性一定程度上解决了企业间的信任问题，并且在联盟链积分平台中有银行作为一个担保人。区块链的点对点交易减少了积分流转、企业对账清算的成本。用户可以在联盟中多家企业自由使用积分，可以将积分转赠给他人。显著扩大了积分的可用范围。用户和企业的使用体验都大幅度提高，这也就是将区块链应用于积分系统的原因。

## 本次项目中的区块链与智能合约
本次项目使用了单群组单机部署方式，各节点之间实时进行共识。

银行节点具有发行积分的权限，作为本项目的一个中心和担保。企业可以根据持有的积分向银行申请承兑现金。用户拥有使用积分、转赠积分的权限。银行发行积分后，若没有企业承兑积分，则整个区块链的积分数量不变。

1. 权限控制：
   使用modifier编写only方法控制各角色的权限。我们用构造函数记录部署合约时的外部账户私钥地址为银行地址，在合约中保存此地址并在调用相关函数时进行限制。并且我们将调用注册企业方法时的地址与企业信息保存在一起，在调用承兑、发放、转增积分函数时限制权限。
2. 信息保存：
...
5. 企业注册：registerCompany(string name);
