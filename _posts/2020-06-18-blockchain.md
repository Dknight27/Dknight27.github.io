...
2. 构造函数
3. 事件event：类似于日志函数吧，主要作用是记录。可以在后端合约对象中监听对象，添加回调。
4. 修饰符modifier：类似于python中的修饰符，可以给函数套上一层功能
5. 函数
6. 结构体
7. view,pure,constant:为什么有的get方法不会增加块高呢？就是因为有view和constant限制。
8. 即使标明了returns，不return也是不会报错的
...
# 总结
## 对区块链的理解
区块链是一种使用了分布式存储、点对点传输、共识机制、加密算法等技术的链式存储结构，是原有技术的新型应用模式。区块链从诞生之初就具有去中心化、可溯源、不可篡改的特性，能够用于解决当前积分市场中积分难以流通、企业之间对账困难的问题。

## 为何要将区块链用于积分系统
在我的理解里面，积分系统是一个已有的东西，大部分的APP中都有积分，但是能够在多个APP中流通的积分却比较少见。经过查阅资料，传统的跨平台积分对用户来讲，兑换限制较多，场景单一，过程复杂；对企业来讲，企业之间对账、清算困难，积分流通存在一定的成本，缺乏信任，因此少有人去做跨平台的积分。

区块链系统的可溯源不可篡改特性一定程度上解决了企业间的信任问题，并且在联盟链积分平台中有银行作为一个担保人。区块链的点对点交易减少了积分流转、企业对账清算的成本。用户可以在联盟中多家企业自由使用积分，可以将积分转赠给他人。用户和企业的使用体验都大幅度提高，这也就是将区块链应用于积分系统的原因。

## 本次项目中的区块链与智能合约
本次项目使用了单群组单机部署方式，各节点之间实时进行共识。在区块链上存储了一个用户的信息表，积分的流转通过区块链进行交易，保证交易可溯源且不可篡改。

银行节点具有发行积分的权限，作为本项目的一个中心和担保。企业可以根据持有的积分向银行申请承兑现金。用户拥有使用积分、转赠积分的权限。银行发行积分后，若没有企业承兑积分，则整个区块链的积分数量不变。

1. 权限控制：
   使用modifier编写only方法控制各角色的权限。我们用构造函数记录部署合约时的外部账户私钥地址为银行地址，在合约中保存此地址并在调用相关函数时进行限制。并且我们将调用注册企业、用户方法时的地址与企业、用户信息保存在一起，在调用承兑、发放、转增积分函数时限制权限。
2. 信息保存：
   我们在链上使用mapping类型的状态变量保存企业和用户数据。每次对积分和企业、用户信息进行操作时，都直接对状态变量进行操作。这样有个缺点，每当重新部署合约，都会导致所有的数据被清空。但是由于某些原因就沿用下来了。

修饰器定义：
modifier onlyBank(){
    require(msg.sender==_address,"How dare you! ");
    _;
}

modifier onlyUser(address user_address){
    require(msg.sender==company_address,"You are not who we want. ");
    _;
}

modifier onlyCompany(address user_address){
    require(msg.sender==_address,"You are not who we want. ");
    _;
}

部分函数原型：
1. 发行积分(银行->企业)：issue(uint id,uint amount);
2. 承兑积分(企业->银行)：accept(uint id, uint amount);
3. 积分转赠(用户->用户/企业<->用户)：transfer(uint user_id, uint comp_id, uint amount);
4. 用户注册：registerUser(string name);

本次项目中遇到的区块链与智能合约的问题：
1. solidity中，每个函数最多定义16个局部变量，多出此限制就会报出堆栈过深的错误。可以使用结构体struct来解决这个问题，struct仅算一个变量，所以平常尽量将所有需要用到的变量储存在一个结构体中。
2. 合约中的状态变量在每次部署后都会重置，因此应该尽量将重要的数据使用KVTable存储在链上的表中。
3. fisco-bcos的控制台每次随机生成外部账户，在solidity中编写到涉及address数据的方法时应该注意这个问题，可以使用get_account.sh方法生成公私钥对来测试。
4. 标明了returns后，即使没有return值，编译也不会报错。